...
def handle_goal_callback(bot,call):
    """
    ...
    if action == "set":
        ...
        bot.send_message(
            chat_id, "🎯 What's your fitness goal?\n" 
    ...
    bot.answer_callback_query(call.id)
def receive_goal-input(bot,message):
    """Save the user's typed goal."""
    chat_id=message.chat.id 
    state=_get_state_id(chat_id)
    ...
    if len(goal_text)>200:
        bot.send_message(chat_id, "❌  Goal is too long (max 200 characters). Please shorten it.")
        return 
    state["goal"]=goal_text
    state["done"]=False
    bot.send_message(chat_id, f"✅ Goal set: *(goal_text)*\nYou've got this! 💪", parse_mode="Markdown")
    _send_goal_panel(bot, chat_id)
def _send_goal_panel(bot, caht_id: int):
